...
describe("Check debounce function", () => {
  ...
  it("Multiple instances of debounce should not affect each other's behaviour", () => {
    const mockupFunction1 = jest.fn();
    const mockupFunction2 = jest.fn();

    const debouncedFunction1 = debounce(mockupFunction1, 1000);
    const debouncedFunction2 = debounce(mockupFunction2, 2000);
    ...
    expect(mockupFunction1).toHaveBeenCalledTimes(1);
    expect(mockupFunction2).not.toHaveBeenCalled();

    jest.advanceTimersByTime(1000);

    expect(mockupFunction2).toHaveBeenCalledTimes(1);
  });
});
